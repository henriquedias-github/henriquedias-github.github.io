...
window.addEventListener('scroll', function() {
    var cestinha = document.querySelector('.car');
    var scrollPosition = window.scrollY;
  
    // Defina a altura mínima para a rolagem afetar a cestinha
    var limiteRolagem = 200; // Ajuste conforme necessário
  
    if (scrollPosition > limiteRolagem) {
      // Caso a rolagem seja maior que o limite, ela permanecerá fixada no final da página
      cestinha.style.top = (window.innerHeight - cestinha.offsetHeight - 10) + 'px';
    } else {
      // Caso contrário, ela volta à posição original
      cestinha.style.top = '90%';
    }
  });
